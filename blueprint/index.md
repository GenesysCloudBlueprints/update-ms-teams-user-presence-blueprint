---
title: "Update the presence of a Microsoft Teams user based upon a Genesys Cloud user presence update"
author: yuri.yeti
indextype: blueprint
icon: blueprint
...
3. From the [update-ms-teams-user-presence-blueprint](https://github.com/GenesysCloudBlueprints/update-ms-teams-user-presence-blueprint) GitHub repository, download the Office-365-Auth.customAuth.json file.
...
1. From the [update-ms-teams-user-presence-blueprint repo](https://github.com/GenesysCloudBlueprints/update-ms-teams-user-presence-blueprint) GitHub repository, download the Find-Teams-User-Id-Multiple-externalIds.custom.json file.
...
1. From the [update-ms-teams-user-presence-blueprint repo](https://github.com/GenesysCloudBlueprints/update-ms-teams-user-presence-blueprint) GitHub repository, download the Update-Teams-User-Presence.custom.json file.
...
This solution includes three Architect workflows that use the data actions you just created. These workflows update the user's presence in Microsoft Teams:

* The **GC User Set MS Teams to Available_v7-0.i3WorkFlow** workflow is triggered when a GC user's presence changes to "Available". This workflow sets the user's presence in Microsoft Teams to `Available`.
* The **GC User Set MS Teams to Busy_v5-0.i3WorkFlow** workflow is triggered when a GC user's presence changes to "On Queue", "Busy", or "Meeting". This workflow sets the user's presence in Microsoft Teams to `Busy`.
* The **GC User Set MS Teams to Away_v5-0.i3WorkFlow** workflow is triggered when a GC user's presence changes to "Away" or "Break". This workflow sets the user's presence in Microsoft Teams to `Away`.
...
5. Select the downloaded **GC User Set MS Teams to Available_v7-0.i3WorkFlow** file. Click **Import**.
...
11. Select the downloaded **GC User Set MS Teams to Busy_v5-0.i3WorkFlow** file. Click **Import**.
...
13. Download the **GC User Set MS Teams to Away_v5-0.i3WorkFlow** file from the [update-ms-teams-user-presence-blueprint](https://github.com/GenesysCloudBlueprints/update-ms-teams-user-presence-blueprint) GitHub repository.  

14. In Genesys Cloud, navigate to **Admin** > **Architect** > **Flows:Workflow** and click **Add**.

  ![Import the script](images/AddWorkflow1.png "Import the script")

15. Name your workflow and click **Create**.

 ![Name your workflow](images/NameWorkflow3.png "Name your workflow")

16. From the **Save** menu, click **Import**.

  ![Import your workflow](images/ImportWorkflow3.png "Import your workflow")

17. Select the downloaded **GC User Set MS Teams to Away_v5-0.i3WorkFlow** file. Click **Import**.

  ![Import your workflow file](images/SelectWorkflow3ImportFile.png "Import your workflow file")

18. Review your workflow. Copy the workflow ID from the URL and save it. You will need it to create the event orchestration trigger. After you have reviewed your workflow, click **Save** and then click **Publish**.
...
1. From the [update-ms-teams-user-presence-blueprint](https://github.com/GenesysCloudBlueprints/update-ms-teams-user-presence-blueprint) GitHub repository, download the Genesys Cloud Event Orchestration Trigger API's.postman_collection.json file.
...
8. Click the **Body** tab. Be sure to change your API domain to match the AWS region your Genesys Cloud organization is hosted. Replace **my-workflow-id** with the id of the **GC User Set MS Teams to Available** Architect workflow that you created earlier in this blueprint. Click **Send**.
...
9. To create the second trigger, replace **my-workflow-id** with the ID of the **GC User Set MS Teams to Busy** Architect workflow that you created earlier in this blueprint. Repeat the actions defined in step 8 and use the JSON body sample below to match the screenshot.  Click **Send**.
...
10. To create the third trigger, replace **my-workflow-id** with the ID of the **GC User Set MS Teams to Away** Architect workflow that you created earlier in this blueprint. Repeat the actions defined in step 8 and use the JSON body sample below to match the screenshot.  Click **Send**.
